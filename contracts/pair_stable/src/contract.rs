...
pub fn query_config(deps: Deps, env: Env) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        params: Some(to_binary(&StablePoolParams {
            amp: compute_current_amp(&config, &env)? / AMP_PRECISION,
        })?),
    })
}
...
fn start_changing_amp(
    ...
    if next_amp_with_precision < current_amp
        && next_amp_with_precision * MAX_AMP_CHANGE < current_amp
        || next_amp_with_precision > current_amp
